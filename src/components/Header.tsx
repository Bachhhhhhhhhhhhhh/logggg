...
export function Header() {
  ...
  return (
    <header
      className="relative z-30 border-b border-line bg-panel/80 backdrop-blur-xl"
      style={{ paddingTop: 'env(safe-area-inset-top)' }}
    >
      <div className="h-px gold-line" />
      <div className="flex items-center gap-2 px-3 py-2 md:gap-5">
        <div className="flex min-w-0 items-center gap-2 md:min-w-[190px] md:gap-3">
          <span className="relative inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-accent/30 bg-gradient-to-br from-[#2a2114] to-[#0c0e14] text-accent md:h-9 md:w-9">
          ...
          <div className="leading-none">
            <h1 className="font-display text-[22px] leading-none tracking-[0.06em] text-ink md:text-[26px]">
              Memory
            </h1>
        ...
        <button
          ...
          className="ml-auto inline-flex h-10 min-w-10 items-center justify-center rounded-full border border-line-strong bg-canvas text-faint md:mx-auto md:h-9 md:max-w-xl md:flex-1 md:justify-start md:gap-3 md:px-3 md:text-left"
        ...
        >
          ...
          <span className="hidden flex-1 text-[12.5px] md:inline">
            Search memory — describe what you need...
          </span>
        ...
        <button
          ...
          className="inline-flex h-10 items-center gap-1.5 rounded-full border border-accent/25 px-2.5 text-[11px] md:hidden"
        >
          <span className={live ? 'live-dot' : 'h-1.5 w-1.5 rounded-full bg-faint'} />
          <span className="font-mono text-accent">{eps.toFixed(1)}</span>
        </button>

        <button
          type="button"
          onClick={() => setSettingsOpen(true)}
          className="inline-flex h-10 min-w-10 items-center justify-center rounded-full border border-line text-muted md:hidden"
          aria-label="Settings"
        >
          ⚙
        </button>

        <div className="ml-0 hidden items-center gap-3 md:flex">
